...
function build_cpu_liouvillian(t::Float64)
    ...
    H = spzeros(T, D, D)
    a_ops = [embed(a, m) for m in 1:M]
    n_ops = [embed(n_op, m) for m in 1:M]
...
function run_gpu_rk4()
    ...
    elem_aa_dag = CuDensityMat.create_elementary_operator(ws, [d, d],
    ...
    for n in 0:M-1, m in 0:M-1
        ...
        coupling_elems[(n, m)] = CuDensityMat.create_elementary_operator(ws, [d, d],
            CUDA.CuVector{T}(vec(c)))
    end
    ...
    CuDensityMat.append_elementary_product!(det_term_1, [elem_n],
        Int32[0], Int32[0]; coefficient=ComplexF64(1.0),
        coefficient_callback=det1_cb, coefficient_gradient_callback=det1_gcb)

    det_term_2 = CuDensityMat.create_operator_term(ws, dims)
    CuDensityMat.append_elementary_product!(det_term_2, [elem_n],
        Int32[1], Int32[0]; coefficient=ComplexF64(1.0),
        coefficient_callback=det2_cb, coefficient_gradient_callback=det2_gcb)
    ...
    for n in 0:M-1, m in 0:M-1
        n == m && continue
        CuDensityMat.append_elementary_product!(coupling_term,
            [coupling_elems[(n, m)]], Int32[n, m], Int32[0, 0];
            coefficient=ComplexF64(1.0),
            coefficient_callback=coup_cb, coefficient_gradient_callback=coup_gcb)
    end
...
function main()
    ...
    for i in 1:D
        ...
        if abs(cpu_p) > 1e-10 || abs(gpu_p) > 1e-10
            println("  $(rpad(label, 10)) | $(lpad(round(cpu_p, digits=8), 13)) | $(lpad(round(gpu_p, digits=8), 13)) | $(round(d_val, sigdigits=3))")
        end
    end
