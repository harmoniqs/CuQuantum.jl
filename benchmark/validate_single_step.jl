...
H = spzeros(T, D, D)
a_ops = [embed(a_s, m) for m = 1:M]
n_ops = [embed(n_s, m) for m = 1:M]
...
elem_aa_dag =
    CuDensityMat.create_elementary_operator(ws, [d, d], CUDA.CuVector{T}(vec(aa_dag_f)))
...
for (val, i, j) in diffs_offdiag[1:5]
    println(
        "  [$i,$j] |diff|=$(round(val, sigdigits=6))  CPU=$(round(rhodot_cpu_mat[i,j], sigdigits=6))  GPU=$(round(rhodot_gpu_mat[i,j], sigdigits=6))",
    )
end
