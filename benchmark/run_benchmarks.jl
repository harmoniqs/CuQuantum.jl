...
function build_cudensitymat_system(M::Int, d::Int)
    ...
    n_mat = T[a_dag[i,k]*a_matrix[k,j] for i in 1:d, j in 1:d, k in 1:d] |>
            x -> dropdims(sum(x, dims=3), dims=3)
    ...
    for n in 0:M-1, m in 0:M-1
        n == m && continue
        CuDensityMat.append_elementary_product!(
            coupling_term, [coupling_elems[(n, m)]],
            Int32[n, m], Int32[0, 0];
    ...
    CuDensityMat.append_term!(liouvillian, diss_sandwich;
        duality=0, coefficient=ComplexF64(γ))
    CuDensityMat.append_term!(liouvillian, diss_number;
...
"""
function benchmark_cudensitymat(M::Int, d::Int;
        n_warmup = M >= 9 ? 1 : 3,
...
function build_cpu_system(M::Int, d::Int)
    ...
    function embed(op_single, mode, M, d)
        ...
        mats = [i == mode ? op_single : Matrix{T}(I, d, d) for i in 1:M]
        kron_result = mats[1]
        for i in 2:M
    ...
    H = zeros(T, D, D)
    a_ops = [embed(a_single, m, M, d) for m in 1:M]
    n_ops = [embed(n_single, m, M, d) for m in 1:M]
    ...
    for m in 1:M
        am = a_ops[m]
        nm = n_ops[m]
        L_super += γ * (kron(conj(am), am) - 0.5*kron(eye, nm) - 0.5*kron(transpose(nm), eye))
    end
...
function build_cpu_sparse_system(M::Int, d::Int)
    ...
    function embed_sparse(op, mode, M, d)
        mats = [i == mode ? op : sparse(T(1)*I, d, d) for i in 1:M]
        result = mats[1]
        for i in 2:M
    ...
    H = spzeros(T, D, D)
    a_ops = [embed_sparse(a_single, m, M, d) for m in 1:M]
    n_ops = [embed_sparse(n_single, m, M, d) for m in 1:M]
    for m in 1:M
    ...
    for m in 1:M
        am = a_ops[m]
        nm = n_ops[m]
        L_super += γ * (kron(conj(am), am) - 0.5*kron(eye_D, nm) - 0.5*kron(transpose(nm), eye_D))
    end
...
function main()
    ...
    if CUDA.functional()
        ...
        for M_try in [8, 9]
            ...
            if free > needed
                ...
                println("Skipping M=$M_try (D=$(D_try)): need ~$(round(needed / 1e9, digits=1)) GB, have $(round(free / 1e9, digits=1)) GB free")
            end
        end
    end
    ...
    println("GPU: $gpu_name | CPU: $(cpu_cores) threads | $(round(Sys.total_memory()/1e9, digits=1)) GB RAM")
    println("=" ^ 78)
    println()
    ...
    for M in M_values
        D = d^M
        println("--- M=$M cavities, Hilbert dim D=$D, ρ size $(D)×$(D) = $(D^2) elements ---")
        ...
        if !isnan(gpu_med)
            println("$(round(gpu_med, digits=3)) ms (min=$(round(gpu_min, digits=3)), max=$(round(gpu_max, digits=3)))")
        end
        ...
        if !isnan(cpu_dense_med)
            println("$(round(cpu_dense_med, digits=3)) ms (min=$(round(cpu_dense_min, digits=3)), max=$(round(cpu_dense_max, digits=3)))")
        ...
        if !isnan(cpu_sparse_med)
            println("$(round(cpu_sparse_med, digits=3)) ms (min=$(round(cpu_sparse_min, digits=3)), max=$(round(cpu_sparse_max, digits=3)))")
        ...
        push!(results, (M=M, D=D, gpu_ms=gpu_med, cpu_dense_ms=cpu_dense_med, cpu_sparse_ms=cpu_sparse_med))
        println()
    end
    ...
    open(csv_file, "w") do io
        ...
        for r in results
            println(io, "$(r.M),$(r.D),$(r.D^2),$(r.gpu_ms),$(r.cpu_dense_ms),$(r.cpu_sparse_ms)")
        end
    end
